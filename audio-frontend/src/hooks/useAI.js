...
export default function AudioWorkspace({ onBack }) {
  ...
  const handleTranscribe = async () => {
    ...
    setIsProcessing(true);
    setProcessLog("🧠 [AI] Analyzing Japanese audio & generating subtitles...");

    try {
      const audioUrl = URL.createObjectURL(audioFile);
      
      // +++ الإصلاح 1: توجيه الذكاء الاصطناعي بشكل دقيق للغة اليابانية +++
      const result = await transcriber(audioUrl, {
        chunk_length_s: 30,
        stride_length_s: 5,
        return_timestamps: true,
        language: "japanese",  // إجبار المحرك على اللغة اليابانية
        task: "transcribe"     // مهمة التفريغ وليس الترجمة للإنجليزية
      });

      console.log("AI Output Data:", result); // للتحقق من المخرجات في الـ Console

      if (result && result.chunks) {
        const realData = result.chunks.map((chunk, index) => ({
          id: index,
          start: chunk.timestamp[0], 
          end: chunk.timestamp[1] || chunk.timestamp[0] + 3, // إذا لم يجد النهاية، يضيف 3 ثوانٍ
          text: chunk.text 
        }));
        ...
        <aside className="w-72 shrink-0 bg-[#060606] border-r border-gray-950 p-4 flex flex-col gap-3 overflow-y-auto min-w-0 relative">
          ...
          <button onClick={handleTranscribe} disabled={isProcessing || !isReady} className={`w-full bg-[#0a0a0a] border ${(isProcessing || !isReady) ? 'border-gray-900 opacity-50 cursor-not-allowed' : 'border-gray-900 hover:border-purple-500/40'} p-3 rounded-xl text-left transition-all group`}>
            <div className="flex items-center gap-3 mb-1">
              ...
              <span className="text-sm font-semibold text-gray-200">Auto-Transcribe (JP)</span>
            </div>
          ...
          <div className="flex h-16 min-h-[64px] items-center border-b border-gray-900/50 group">
            ...
            <div className="flex-1 h-full relative flex items-center py-1 overflow-visible">
              {transcriptionData.length > 0 && transcriptionData.map((clip) => {
                ...
                return (
                  // +++ الإصلاح 2: إضافة جروب والتلاعب بالـ Z-index و Hover ليظهر النص كاملاً +++
                  <div 
                    key={clip.id}
                    className={`absolute h-8 rounded border flex flex-col justify-center px-2 text-[10px] font-mono transition-colors shadow-sm cursor-text group ${isActive ? 'bg-purple-500/30 border-purple-400 text-white z-20 scale-105' : 'bg-gray-800/50 border-gray-700 text-gray-400 hover:z-30 hover:bg-gray-700'}`}
                    style={{ left: `${leftPos}%`, width: `${Math.max(widthPos, 4)}%` }} // ضمان عرض المربع حتى لو كانت الكلمة سريعة جداً
                  >
                    <span className="truncate group-hover:absolute group-hover:bg-[#111] group-hover:border group-hover:border-purple-500 group-hover:p-2 group-hover:rounded group-hover:z-50 group-hover:w-max group-hover:text-white transition-all">
                      {clip.text}
                    </span>
                  </div>
                );
